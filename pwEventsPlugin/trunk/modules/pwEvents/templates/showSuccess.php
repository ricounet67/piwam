...
<table width="600px" class="details">
  ...
  <tr>
    <th>Détails privée</th>
    <td><?php echo $event->getDescriptionPrivate() ?></td>
  </tr>
  <?php endif ?>
  <tr>
    <th>Adresse</th>
  ...
  <tr>
    <th>Ajouter participant</th>
    <td>
    <form
      action="<?php echo url_for('@event_subscription_register?event_id='.$event->getId()) ?>"
      method="post"><?php echo $formAddMember['member_id'] ?> <input
      type="submit" value="Ajouter" class="add blue button" /></form>
    </td>
  </tr>
  <?php endif ?>
  ...
  <tr>
    <th colspan="2" style="text-align: center">Covoiturage</th>
  </tr>
  <tr>
    <td colspan="2" style="padding: 0px;"><?php if(count($carpool_registers_need) + count($carpool_registers_offer) > 0): ?>
    <table width="100%" class="subTable">
      <tr>
        <td width="50%" style="padding: 0px;"><?php if(count($carpool_registers_offer) > 0): ?>
        <table width="100%" class="subTable">
          <thead>
            <tr>
          ...
          <?php foreach($carpool_registers_offer as $offer): ?>
            <tr>
              <td><?php echo jq_link_to_function($offer->getMember()->getName(),'carpoolMapOpenWindow('.$offer->getMember()->getId().')') ?>
        ...
        <td width="50%" style="padding: 0px;"><?php if(count($carpool_registers_need) > 0): ?>
        <table width="100%" class="subTable">
          <thead>
            <tr>
              <th>Cherche des places</th>
            </tr>
          </thead>
          <tbody>
          <?php foreach($carpool_registers_need as $need): ?>
            <tr>
              <td><?php echo jq_link_to_function($need->getMember()->getName(),'carpoolMapOpenWindow('.$need->getMember()->getId().')') ?>
    ...
    <?php else: ?> <br />
    Personne ne cherche ou ne propose de places de covoiturage.<br />
    <?php endif ?></td>
  </tr>
  ...
  <tr>
    <th colspan="2" style="text-align: center">Carte de l'événement</th>
  </tr>
  <tr>
    <td colspan="2"><?php include_map($carpool_map,array('height'=>'400px'))?>
    <table
      style="border: 1px solid #999; margin-top: 15px; margin-bottom: 10px; width: 100%;"
      class="subTable">
      <tr
        style="font-weight: bold; background-color: #ddd; color: #555;">
        <td colspan="4">Légende&nbsp;</td>
      </tr>
      <tr>
        <td width="20px"><?php echo image_tag(pwEventsPluginUtil::IMAGES_PATH.'home.png',array('size'=>'24x24')) ?></td>
        <td>Chez moi</td>
        <td><?php echo image_tag(pwEventsPluginUtil::IMAGES_PATH.'end_flag.png',array('size'=>'24x24')) ?></td>
        <td>Adresse de l'événement</td>
      </tr>
      <tr>
        <td><?php echo image_tag(pwEventsPluginUtil::IMAGES_PATH.'flag_blue.png',array('size'=>'24x24')) ?></td>
        <td>Cherche des places</td>
        <td><?php echo image_tag(pwEventsPluginUtil::IMAGES_PATH.'flag_green.png',array('size'=>'24x24')) ?></td>
        <td>Propose des places</td>
      </tr>
    </table>
    </td>
  </tr>
