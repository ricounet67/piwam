...
<td width="34%" style="text-align:center"><h2><?php echo $currentTitle ?></h2></td>
...
<tr><td ><?php echo $firstWeekMonth+$week ?></td>
<?php foreach($days as $day => $events): ?>
<td  width="14%" height="100px"
	style="border: solid 1px black; padding: 0px; vertical-align: top;" <?php if(date('Y-m-d') == $day): ?>class="dayCellToday"<?php endif ?> >
	<table width="100%" height="100%" style="padding: 0px" >
	<tr height="20px"
			<?php if(intval(date('m',strtotime($day)) == $currentMonth)): ?>class="dayHeaderCurrentMonth"<?php endif ?> >
	...
	<tr ><td>	
			...
			<?php foreach($events as $key => $value): ?>
				<li>&bull;
					<?php echo link_to($value['event']->getName(),'@event_show?id='.$value['event']->getId())//.' à '. $value['event']->getTimeBegin()?>
				</li>
			<?php endforeach ?>
			</ul>
		</td>
